...
class MainForm(QMainWindow, ModbusClient):
    ...
    def initUI(self):  # 初始化程式參數
        sql = f'TRUNCATE public.device_button_setup'
        self.session.execute(sql)
        sql = f"""UPDATE public.device_check_status SET count='3' WHERE item='login'"""
        self.session.execute(sql)
        sql = f"""UPDATE public.device_check_status SET count='30' WHERE item='monitor_time'"""
        self.session.execute(sql)
        sql = f"""UPDATE public.device_check_status SET count='0' WHERE item='buffer'"""
        self.session.execute(sql)
        ...
        sql = f"""SELECT count FROM public.device_check_status WHERE item='alarm_ch1'"""
        count = self.session.execute(sql).fetchall()[0][0]
        self.ui.lineEdit_3.setText(str(count))
        sql = f"""SELECT count FROM public.device_check_status WHERE item='alarm_ch2'"""
        count = self.session.execute(sql).fetchall()[0][0]
        self.ui.lineEdit_16.setText(str(count))
        sql = f"""SELECT count FROM public.device_check_status WHERE item='alarm_ch3'"""
        count = self.session.execute(sql).fetchall()[0][0]
        self.ui.lineEdit_17.setText(str(count))
        sql = f"""SELECT count FROM public.device_check_status WHERE item='alarm_ch4'"""
        count = self.session.execute(sql).fetchall()[0][0]
        self.ui.lineEdit_18.setText(str(count))
        sql = f"""SELECT count FROM public.device_check_status WHERE item='phoenix'"""
        self.alarm_IP = self.session.execute(sql).fetchall()[0][0]
        for i in range(1, 9):
            sql = f"""INSERT INTO public.device_button_setup(
    ...
    def button_setup(self, button_list):  # 主畫面按鈕設定
        ...
        if button_list[1] != "" and check_list[1][3] == check_list[1][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[1]}' WHERE subMainForm='subMainForm2'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[2] != "" and check_list[2][3] == check_list[2][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[2]}' WHERE subMainForm='subMainForm3'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[3] != "" and check_list[3][3] == check_list[3][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[3]}' WHERE subMainForm='subMainForm4'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[4] != "" and check_list[4][3] == check_list[4][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[4]}' WHERE subMainForm='subMainForm5'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[5] != "" and check_list[5][3] == check_list[5][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[5]}' WHERE subMainForm='subMainForm6'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[6] != "" and check_list[6][3] == check_list[6][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[6]}' WHERE subMainForm='subMainForm7'"""
            self.session.execute(sql)
        else:
            pass
        if button_list[7] != "" and check_list[7][3] == check_list[7][0]:
            ...
            sql = f"""UPDATE public.device_button_setup SET serial='{button_list[7]}' WHERE subMainForm='subMainForm8'"""
            self.session.execute(sql)
        else:
            pass
    ...
    def _get_last_data(self, serial):
        data = pd.read_sql(f"""SELECT * FROM public."SN{serial}" order by timedate""", self.session.bind)
        data = data.tail(1)
    ...
    def set_alarm_ch2(self):
        ...
        sql = f'UPDATE public.device_check_status SET count="{alarm_count}" WHERE item="alarm_ch2"'
        self.session.execute(sql)
        self.setting_para_compelete(text)
    ...
    def set_alarm_ch3(self):
        ...
        sql = f'UPDATE public.device_check_status SET count="{alarm_count}" WHERE item="alarm_ch3"'
        self.session.execute(sql)
        self.setting_para_compelete(text)
    ...
    def set_alarm_ch4(self):
        ...
        sql = f'UPDATE public.device_check_status SET count="{alarm_count}" WHERE item="alarm_ch4"'
        self.session.execute(sql)
    ...
    def add_ip_port(self):
        ...
        if ip == "":
            ...
            sql = f"""INSERT INTO public.device_connect (ip,model,serial) VALUES ('{ip}','{model}','{serial}') ON CONFLICT ("ip") DO NOTHING"""
            self.session.execute(sql)
            sql = f"""CREATE TABLE IF NOT EXISTS public."SN{str(serial)}"
                    (
    ...
    def log_Alarm_reset(self, num):
        ...
        sql = f'SELECT * FROM public.device_login WHERE id="{alarm_reset_id}"'
        list1 = self.session.execute(sql).fetchall()
        level = list1[0][2]
        if num == 1:
            sql = f"""INSERT INTO public.device_soft_even_log ("timedate","id","level","message") VALUES ('{str(now_time)}','{str(
    ...
    def Worker(self, ip, serial, status_con):
        try:
            ...
            if int(flow_alert) == 1:
                ...
                if self.ui.checkBox_7.isChecked():
                    if self.alarm_client.connect():
                        sql = f"""SELECT count FROM public.device_check_status WHERE item='colorlight_red'"""
                        colorlight_red = self.session.execute(sql).fetchall()[0][0]
                        sql = f"""SELECT count FROM public.device_check_status WHERE item='colorlight_yellow'"""
                        ...
                        try:
                            if colorlight_red == 1:
                                ...
                                if colorlight_yellow == 0:
                                    ...
                                    sql = f"""UPDATE public.device_check_status SET count='1' WHERE item='colorlight_red'"""
                                    self.session.execute(sql)
            ...
            if ch4 > alarm_ch4:
                ...
                if self.ui.checkBox_7.isChecked():
                    ...
                    pass
            sql = f"""INSERT INTO public.'SN{serial}' ("timedate","0.3um","0.5um","1um","5um","Location","Sample time","HV","TV","Flow","Service") VALUES ('{str(r_time)}','0','0','0','0','{str(location)}','{sample_time_f}','{str(humi)}','{str(temp)}','{str(flow_alert_s)}','{str(status_alert_s)}') ON CONFLICT ("timedate") DO NOTHING"""
            self.session.execute(sql)
            ...
            for k in range(4):
                if particle_size[k] == 0:
                    ...
                    if (particle_size[k] / 1000) >= 1:
                        sql = f"""UPDATE public.SN{serial} SET '{str(int(particle_size[k] / 1000))}um'='{particle_count[k]}' WHERE timedate='{str(r_time)}'"""
                        self.session.execute(sql)
            ...
            level = list1[0][2]
            if flow_alert == 'Alert':
                sql = f"""INSERT INTO public.device_eq_even_log ("timedate","id","level","message") VALUES ('{str(
...
class subMainForm(QDialog, ModbusClient):
    ...
    def update_data(self):
        ...
        sql = f"""SELECT * FROM public."SN{self.serial}" WHERE timedate BETWEEN '{end_time}' AND '{start_time}' ORDER BY 1 DESC"""
